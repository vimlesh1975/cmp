...
Public Class ucTemplatePlaylist
    ...
    Public Sub adjusttimeofrundown()
        ...
        For iadjusttime = 0 To dgvrundown.Rows.Count - 1
            ...
            dgvrundown.Rows(iadjusttime).Cells(6).Value += Val(txtadjusttimeofrundownmin.Text) * 60 + Val(txtadjusttimeofrundownsec.Text)
        Next
    End Sub
    ...
    Private Sub cmdaddtorundown_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdaddtorundown.Click
        ...
        dgvrundown.CurrentRow.Cells(6).Value = 5
    ...
    Sub initialiserundown()
        ...
        dgvrundown.Columns.Add(col4)

        Dim ofd2 As New OpenFileDialog
        ofd2.FileName = "c:/casparcg/mydata/rundown/ad.txt"
        Using sr As StreamReader = New StreamReader(ofd2.FileName)
            dgvrundown.Rows.Clear()
            Dim g As Integer = 0
            Dim li As String
    ...
    Public Sub stopshedule()
        ...
        lblsheduleStatus.Text = "Playlist Stopped"
    ...
    Private Sub cmdstartsheduletemplate_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdstartsheduletemplate.Click
        ...
        For aaa = 0 To dgvrundown.RowCount - 1
            bbb = bbb + dgvrundown.Item(8, aaa).Value
        Next
    ...
    Public Sub sheduletemplate()
        ...
        'rundownrowsselectedchanged()
        tmrsheduletemplatestart.Interval = 100 ' IntervalTill(CType(dgvrundown.Rows(0).Cells(5).Value, Date).TimeOfDay.ToString)
        tmrsheduletemplatestart.Enabled = True
        lblsheduleStatus.Text = "Playlist Started"
    ...
    Private Sub tmrsheduletemplatestart_Tick(ByVal sender As Object, ByVal e As System.EventArgs) Handles tmrsheduletemplatestart.Tick
        ...
        CasparDevice.SendString("cg " & g_int_ChannelNumber & "-" & dgvrundown.CurrentRow.Cells(2).Value & " add " & dgvrundown.CurrentRow.Cells(3).Value & " " & """" & dgvrundown.CurrentRow.Cells(1).Value & """" & " 1 " & """" & CasparCGDataCollection.ToAMCPEscapedXml & """" & vbCrLf)
        colorchange()
        tmrsheduletemplatestart.Interval = dgvrundown.CurrentRow.Cells(6).Value * 1000
20:
        If dgvrundown.CurrentRow.Index = dgvrundown.RowCount - 1 Then
            dgvrundown.CurrentCell = dgvrundown.Rows(0).Cells(0)
        Else
            Dim dd As Integer = dgvrundown.CurrentRow.Index
            Dim i As Integer
            For i = dd + 1 To dgvrundown.RowCount - 1
                If dgvrundown.Rows(i).Cells(0).Value <> "" And dgvrundown.Rows(i).Cells(8).Value = True Then
                    GoTo 50
                End If
            Next
            For i = 0 To dd
...
50:
            dgvrundown.CurrentCell = dgvrundown.Rows(i).Cells(0)
        End If

        'If dgvrundown.CurrentRow.Cells(0).Value = "" Or dgvrundown.CurrentRow.Cells(8).Value = False Then
        '    GoTo 20
        'End If
        rundownrowsselectedchanged()
    End Sub
    Sub colorchange()
        For i = 0 To dgvrundown.RowCount - 1
            With dgvrundown
    ...
    Private Sub CmdSelectAll_Click(sender As Object, e As EventArgs) Handles cmdSelectAll.Click
        ...
        For iplalistselectall = 0 To dgvrundown.RowCount - 1
            dgvrundown.Rows(iplalistselectall).Cells(8).Value = True
        Next
    End Sub
    ...
    Private Sub CmdDeselectAll_Click(sender As Object, e As EventArgs) Handles cmdDeselectAll.Click
        ...
        Dim iplalistselectall As Integer
        For iplalistselectall = 0 To dgvrundown.RowCount - 1
            dgvrundown.Rows(iplalistselectall).Cells(8).Value = False
        Next
    End Sub
End Class
